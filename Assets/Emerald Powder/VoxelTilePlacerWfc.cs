...
public class VoxelTilePlacerWfc : MonoBehaviour
{
    ...
    private void Generate()
    {
        ...
        {
            for (int x = 0 + MapSize.x; x < MapSize.x; x++)
            for (int y = 0 + MapSize.y; y < MapSize.y; y++)
            {
                possibleTiles[x + MapSize.x, y + MapSize.y] = new List<VoxelTile>(TilePrefabs);
            }
            ...
            // Надо менять положение спавна немного в сторону
        }
    ...
    private void PlaceTile(int x, int y)
    {
        ...
        Vector3 position = selectedTile.VoxelSize * selectedTile.TileSideVoxels * new Vector3(x + 0, 0, y + 0);
